...
export const hasClassificationBannerType = (type: "top" | "bottom" | "notInline"): boolean => {
  if (type === "top") {
    return !!document.querySelector("ic-classification-banner[top]");
  }
  if (type === "bottom") {
    return !!document.querySelector("ic-classification-banner:not([inline='true']):not([top='true'])");
  }
  if (type === "notInline") {
